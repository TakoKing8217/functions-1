...
// CODE HERE
const totalPopulation = populations.reduce((prev, curr) => prev + curr);
...
const newOrders = orders.map(
  (priceAndRate) => priceAndRate.price * (priceAndRate.tax + 1)
);
...
const bobsTotal = purchases
  .filter((ownerAndPrices) => ownerAndPrices.owner === "Bob")
  .map((ownerAndPrices) => ownerAndPrices.price)
